...
"""Tests for TU masking utilities (binary STE masking)."""
...
# --- Tests for compute_binary_masks with explicit is_multi_tu flag ---


def test_compute_binary_masks_single_tu_requires_1d():
    """is_multi_tu=False requires 1D tu_indices."""
    tu_indices = jnp.array([0, 1, 2])  # 1D - single TU per input
    tu_log_alpha = jnp.zeros(4)

    masks = compute_binary_masks(tu_indices, tu_log_alpha, is_multi_tu=False)
    assert masks.shape == (3,), f"Expected (3,), got {masks.shape}"


def test_compute_binary_masks_multi_tu_requires_2d():
    """is_multi_tu=True requires 2D tu_indices."""
    tu_indices = jnp.array([[0, 1], [1, 2], [2, 3]])  # 2D - multi TU per input
    tu_log_alpha = jnp.zeros(4)

    masks = compute_binary_masks(tu_indices, tu_log_alpha, is_multi_tu=True)
    assert masks.shape == (3,), f"Expected (3,), got {masks.shape}"


def test_compute_binary_masks_single_tu_rejects_2d():
    ...
    with pytest.raises(AssertionError, match=r"is_multi_tu=False but tu_indices.ndim=2"):
        compute_binary_masks(tu_indices, tu_log_alpha, is_multi_tu=False)


def test_compute_binary_masks_multi_tu_rejects_1d():
    ...
    with pytest.raises(AssertionError, match=r"is_multi_tu=True but tu_indices.ndim=1"):
        compute_binary_masks(tu_indices, tu_log_alpha, is_multi_tu=True)
