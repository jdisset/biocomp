...
x1ratios = raw_ratios
x2ratios = raw_ratios[::-1].copy()
bratios = np.ones((8,)) / 8
x1ratios = x1ratios / np.sum(x1ratios)
x2ratios = x2ratios / np.sum(x2ratios)
bratios = bratios / np.sum(bratios)
...
def complex_twolayers_advanced_stack_assertions(
    ...
    # not sure which node is which so I just test a random input
...
def test_complex_twolayers_quantization_masks(lib, complex_twolayers_design_network):
    ...
    def dummy_transform_inv(value, qrate, rv_inner=0, rv_outer=0):
        # Forward: output = (input + qrate + rv_inner) * 8 + rv_outer
        # Inverse: input = (output - rv_outer) / 8 - qrate - rv_inner
        ...
        # Step 1: Remove outer random variable
        temp = jnp.sum(value) - jnp.sum(rv_outer)

        # Step 2: Undo the *8 multiplication
        temp = temp / 8

        # Step 3: Remove qrate and rv_inner
    ...
    def inverse(x, marker_ratio, qrate):
        #  inverse stack is:
        # x -> inv_tl -> inv_tr -> inv_src -> inv_agg
    ...
    def dummy_ern(ern_name, positive_rna, negative_prt, layer_id=0, rv=0):
        ...
        print(f"ERN name: {ern_name}, id: {ern_name_id}, affinity: {affinity_val}")
    ...
    # initialize X as uniform random values
