...
"""Network-aware smooth line plotting.

Generic 1D/2D smooth kernels live in :mod:`jeanplot.plots.smooth_1d` and
:mod:`jeanplot.plots.smooth_2d`. This module hosts only the biocomp
...
def smooth_line_plot(
    x, y, network, rescaler, ax,
    res=200, xmin=0, xmax=1, vlims=(None, None),
    slice_at=None, label=None, color=None, lw=1, tree=None,
    marker=None, markevery=20, with_quantiles=None,
    sample_quantiles_at=None, **kw,
):
    ...
    vmin, vmax = vlims
    return [vmin if vmin is not None else np.nanmin(z), vmax if vmax is not None else np.nanmax(z)]


def smooth_line_slices(
    x, y, network, rescaler, slices, input_order,
    axes=None, ax=None, xmin=0, xmax=None,
    color_mode="inner_slice", markers=None, **kwargs,
):
    if markers is None:
    ...
    input_order, _, input_names, _ = get_reordered_protein_names(network, input_order=input_order)
    x = x[:, input_order]
    ...
    for i, outsl in enumerate(outerslices):
        ...
        for j, insl in enumerate(innerslices):
            ...
            vl = smooth_line_plot(
                x, y, network, rescaler,
                ax=iax, slice_at=[insl, outsl], input_order=input_order,
                xmax=x.max(),
                label=f"{input_names[input_order[1]]} ≈ {format_powers(rescaler.inv(insl), n_decimals=0)}",
                tree=tree, color=color, marker=markers[i], **kwargs,
            )
    ...
    if same_ax:
        setup_transformed_axis(
            iax, xaxis_lims=[xmin, xmax], yaxis_lims=vlims,
            rescaler=rescaler, margins=0.05, **kwargs,
        )
