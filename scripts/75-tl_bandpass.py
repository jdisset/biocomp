...
transcription_units_bp = {
    tu_name: tu for tu_name, tu in tus_bp.items() if tu_name in ut.flatten(aggregations_bp)
}

sources_bp
# expand source to be a list of (TU_id, TU_name, position)
raw_sources_bp = [(source, tu_name, 0) for tu_name, tu in sources_bp.items() for source in tu]
raw_aggregations_bp = [(i, source, 1.0) for i, agg in enumerate(aggregations_bp) for source in agg]
transcription_units_bp


n_bp = bc.Network.from_raw(
    lib, 'bp_attempt', transcription_units_bp, raw_sources_bp, raw_aggregations_bp
)
...
# ut.save(NETWORK, Path('~/.biocompiler/networks/bp_v1.pkl').expanduser())
...
### {{{                      --     load parameters     --
archive_path = Path('~/.biocompdir/training_archives/2023-09-21_expert-paper-5.pkl')
...
for i, epoch_key in enumerate(jax.random.split(key, training_config['epochs']), 1):
    ...
    params, opt_state, epoch_history = jax.jit(epoch_step)(params, opt_state, epoch_key, xb, yb)
...
# plot experiments
TOPBOUND = 10000
x = np.linspace(10, TOPBOUND, 10000)
fig, ax = du.mkfig(1, 1, (5, 5), dpi=300)
y = x
ax.plot(x, y, lw=2, label='y=x')
y2 = (100 + x) * 0.2
ax.plot(x, y2, lw=2, label='y=(100+x)*0.2')
y3 = (TOPBOUND - x)
ax.plot(x, y3, lw=2, label='y=(10000-x)*2')
y4 = (TOPBOUND - x) * 5
ax.plot(x, y4, lw=2, label='y=(10000-x)*5')
y5 = (TOPBOUND - x) * 0.5
ax.plot(x, y5, lw=2, label='y=(10000-x)*0.5')
y6 = x ** -5
ax.plot(x, y6, lw=2, label='y=10000-(x*0.5)')
# y7 = (0.1*x) ** 2
# ax.plot(x, y7, lw=2, label='y=(100+x)*3')
...
##
p = 4
x = np.geomspace(1, 10**p, 100000)
x = np.linspace(1, 10**p, 100000)
fig, ax = du.mkfig(1, 1, (3, 3), dpi=300)
for i in range(p+1):
    y = 10**(i) - x
...
# ax.set_xscale('log')
# ax.set_yscale('log')
ax.set_xlim(10, 10**(p)+10**(p-1))
ax.set_ylim(10, 10**(p)+10**(p-1))
